    ...
    initializeStore() {
      ...
      if (savedState) {
        ...
        for (const key in this.settings) {
          if (
            ...
            parsedSavedState.settings[this.settings[key].id].hasOwnProperty(
              "value"
            )
          ) {
            this.settings[key].value =
              parsedSavedState.settings[this.settings[key].id].value;
          }
        }
        ...
        for (const key in this.callbacks) {
          if (
            ...
            parsedSavedState.callbacks[this.callbacks[key].id].hasOwnProperty(
              "value"
            )
          ) {
            this.callbacks[key].value =
              parsedSavedState.callbacks[this.callbacks[key].id].value;
          }
        }
      }
      ...
      this.$subscribe((_, state) => {
        ...
        for (const key in state.settings) {
          savedSettings[state.settings[key].id] = {
            value: state.settings[key].value,
          };
        }

        for (const key in state.preferences) {
          savedPreferences[state.preferences[key].id] = {
            value: state.preferences[key].value,
          };
        }

        for (const key in state.callbacks) {
          savedCallbacks[state.callbacks[key].id] = {
            value: state.callbacks[key].value,
          };
        }
